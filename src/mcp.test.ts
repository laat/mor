...
describe('notes_patch', () => {
  it('patches content and shows diff', async () => {
    ...
    expect(text).toContain('Patched: Patch Target');
    expect(text).toContain('--- content diff ---');
  });
  ...
  it('deletes text with empty new_str', async () => {
    ...
    expect(text).toContain('Patched: Patch Delete');

    const { text: readText } = await callTool('notes_read', {
      ids: [mem.id],
    });
    expect(readText).toContain('keep keep');
  });
  ...
  it('returns error when old_str is not unique', async () => {
    const mem = await ops.add({
      title: 'Ambiguous',
      content: 'aaa bbb aaa',
    });
    ...
    expect(isError).toBe(true);
    expect(text).toContain('old_str appears multiple times');
  });
