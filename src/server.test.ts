...
describe("HTTP Server", () => {
  ...
  it("GET /memories/:id reads a memory by UUID", async () => {
    const { json: created } = await req("POST", "/memories", { title: "Read Me", content: "content here" });
  ...
  it("GET /memories/:prefix reads by UUID prefix", async () => {
    const { json: created } = await req("POST", "/memories", { title: "Prefix Test", content: "prefix content" });
  ...
  it("GET /memories/search?q=... searches memories", async () => {
    await req("POST", "/memories", { title: "TypeScript Patterns", content: "generics and types" });
    await req("POST", "/memories", { title: "Cooking Recipe", content: "pasta carbonara" });
  ...
  it("PUT /memories/:id updates a memory", async () => {
    const { json: created } = await req("POST", "/memories", { title: "Old Title", content: "old content" });
    const id = created.data.id;

    const { status, json } = await req("PUT", `/memories/${id}`, { title: "New Title", content: "new content" });
    expect(status).toBe(200);
  ...
  it("DELETE /memories/:id removes a memory", async () => {
    const { json: created } = await req("POST", "/memories", { title: "Delete Me", content: "gone soon" });
    const id = created.data.id;
  ...
  it("returns 404 for unknown memory", async () => {
    const { status, json } = await req("GET", "/memories/nonexistent-id-00000000");
  ...
  it("round-trip: add → search → read → update → delete", async () => {
    ...
    // Search
    const { json: searchRes } = await req("GET", "/memories/search?q=round+trip");
    ...
    // Update
    const { json: updateRes } = await req("PUT", `/memories/${id}`, { content: "updated content" });
...
describe("HTTP Server Auth", () => {
  ...
  beforeEach(async () => {
    authServer = startServer(config, { port: 0, host: "127.0.0.1", token: "secret123" });
